"""
turbo_moc.plotting_mpl -- matplotlib plots built from turbo_moc.design_nozzle's result
...
is the matplotlib half; turbo_moc.plotting_plotly provides the same functions
...
compare. Used by turbo_moc.app's "pin as reference" workflow, but standalone too.
"""

import matplotlib.pyplot as plt
import jaxprop as jxp

from turbo_moc._phase_diagram import get_fluid
...
# Called explicitly here (not just relying on turbo_moc.solvers.conventional/mln
...
def plot_nozzle_contour(data, ax=None, mirror=True, show_sauer=True, show_kernel=True,
    ...
    axisymmetric half-solve throughout turbo_moc, same convention as the rest of
...
def plot_ts_diagram(data, ax=None, show_dome=True, show_spinodal=False, reference=None,
    ...
    turbo_moc._phase_diagram.get_fluid caches the Fluid per fluid name so the
...
def plot_blade(blade_data, ax=None, n_blades=2, show_control_points=True,
                ...
                reference=None, label="Current", reference_label="Reference"):
    """Cascade view of the fitted blade profile -- turbo_moc.geometry.
    ...
    distinct marker -- used by turbo_moc.app's interactive control-point editor
    (see turbo_moc.geometry.move_control_point_along_normal) to show which
...
def plot_rotor_blade(data, ax=None, show_junctions=True,
                      reference=None, label="Current", reference_label="Reference"):
    """
    Pressure/suction surfaces from turbo_moc.rotor.design_rotor_vortex_blade
...
def plot_rotor_vortex_blade(data, ax=None, show_surfaces=True, show_mach_lines=False,
                              reference=None, label="Current", reference_label="Reference"):
    """Filled vortex-flow rotor blade (turbo_moc.rotor.design_rotor_vortex_blade's
...
def plot_radial_cascade(radial_data, ax=None, show_radius_circles=True):
    """Full annular cascade view of turbo_moc.geometry.wrap_blade_radial(...) /
...
def plot_meridional_view(meridional_data, ax=None, units="mm"):
    ...
    from turbo_moc.geometry.build_meridional_view(...): hub/tip lines for each
