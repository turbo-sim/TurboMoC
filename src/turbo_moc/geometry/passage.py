"""
turbo_moc.geometry.passage -- single flow-passage (one blade-to-blade channel)
...
- Rotor: pressure(c) and suction(c)+pitch+translate are literally the
  ...
  never meeting -- see turbo_moc.rotor.vortex_blade's own module docstring),
...
turbo_moc.geometry.radial's log-spiral wrap, the same mapping already used to
...
Requires cadquery (not a core turbo_moc dependency -- import kept local, same
convention as turbo_moc.geometry.blade's STEP export functions).
"""
...
def _passage_geometry(blade_data, pitch, source, inlet_chords, outlet_chords,
    ...
    if source == "stator":
        ...
        # turbo_moc.app's matching note on this swap.
...
def extract_axial_passage_2d(blade_data, pitch, face_path, solid_path=None,
    ...
    pitch : float
        ...
        scaled from its own nondimensional r* (see turbo_moc.app's
    ...
    source : str
        ...
        turbo_moc/geometry/radial.py's module docstring on the two blades'
...
def extract_radial_passage_2d(blade_data, pitch, r1, r2, face_path, solid_path=None,
    ...
    turbo_moc.geometry.radial's log-spiral wrap -- the same mapping already
    ...
    mapping's reference point/scale (see turbo_moc.geometry.radial's
