...
from turbo_moc.core.classes import (
...
from turbo_moc.io.post_process import printProgress
...
def march_line_jax_step(carry, _):
    """Vectorised MoC step with predictor-corrector (identical to turbo_moc/)."""
...
class MOCSolverMLN:
    ...
    Stages 1, 2, 4 are identical to MOCSolver in turbo_moc/.
    ...
    def __init__(self, config: dict, show_plot=False, fluid_manager: 'FluidManager' = None):
        ...
        # See turbo_moc.solvers.conventional.MOCSolver's __init__ for why a
    ...
    def _stage1_sauer_smooth(self, progress_callback=None):
        """Real Sauer transonic sonic line (identical to turbo_moc/)."""
    ...
    # ------------------------------------------------------------------
    # STAGE 4 — Reflex zone (identical to turbo_moc/)
    # ------------------------------------------------------------------
    ...
    def run_stage_4_reflex(self, refine_near_corner=False, n_sub=8, n_segments=3, progress_callback=None):
        ...
        # turbo_moc.solvers.conventional.MOCSolver.run_stage_4_reflex, ported here
    ...
    def build_result_data(self, extra_meta=None):
        ...
        see turbo_moc.solvers.conventional.MOCSolver.build_result_data (same
        schema, reused as-is by turbo_moc.api)."""
    ...
    def export_full_result_data(self, filename="turbo_moc_result_data.json", extra_meta=None):
