"""
turbo_moc.plotting_plotly -- same functions as turbo_moc.plotting_mpl, returning
...
its plotting_plotly.py) -- a callback calls turbo_moc.design_nozzle(...) once,
...
plotting_mpl's module docstring. Used by turbo_moc.app's "pin as reference"
...
def _apply_journal_style(fig):
    ...
    call turbo_moc.plotting_mpl makes for the matplotlib/download half of this
...
def plot_rotor_vortex_blade(data, show_surfaces=True, show_mach_lines=False,
                              reference=None, label="Current", reference_label="Reference"):
    """Vortex-flow rotor blade (turbo_moc.rotor.design_rotor_vortex_blade,
...
def plot_radial_cascade(radial_data, show_radius_circles=True, clip_quadrant=True):
    ...
    built by turbo_moc.app's Phase 5 "Both" source) to overlay the two rows the
...
def plot_meridional_view(meridional_data, units="mm", z_range=None):
    ...
    turbo_moc.geometry.build_meridional_view(...): hub/tip lines for each row
